...
data_casen_2020 <- data_casen_2020 %>% 
  select(edad,
         sexo,
         region, 
         expr, 
  ...
  mutate(año = 2020,
         ...
         nacionalidad_procedencia = ifelse( 
           (r1b %in% c(1,2)) & (r2  %in% c(1,2)),
           "Chilena", 
           ifelse(
             (r1b == 3) & (r2 == 4) & (r2espp_cod == 513), 
             "Venezolana",
             "Otra"
           )
         ),
         ...
         ocupacion_formal = case_when(
           s13 %in% c(1,3) & o32 == 1  ~ "1", # Ocupación Formal
