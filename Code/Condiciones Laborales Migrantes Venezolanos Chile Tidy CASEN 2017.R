...
data_casen_2017 <- data_casen_2017 %>% 
  select(edad,
         sexo,
         region, 
         expr, 
  ...
  rename(r1bespp_cod = r1b_p_cod,  
         r2espp_cod = r2_p_cod, 
         ingreso = yoprcor) %>%
  mutate(año = 2017,
         ############### Inicio de Transformación de Valores Missing en NA ##############
         e6a = replace(e6a, e6a == 99, NA),
         y2_hrs = replace(y2_hrs, y2_hrs == 999, NA),   
         ...
         ocupacion_formal = case_when(
           s12 %in% c(1,2,3,4,5,7) & o29 == 1 ~ "1", # Ocupación Formal
           o15 == 9  | s12 == 8 | o29 == 7 & o15 %in% c(3,4,5,6,7,8) ~ "0" , # Ocupación Informal
           TRUE ~ NA_character_
         ), 
         ocupados_cotizacion = case_when(
           o29 %in% c(1,2,3,4,5,6) ~ "1", # Cotiza en el Sistema de Pensiones
           o29 == 7 ~ "0", # No Cotiza en el Sistema de Pensiones
           TRUE ~ NA_character_
         ), 
         y2_hrs = (y2_hrs / 4)
  ) %>% 
