...
data_casen_2015 <- data_casen_2015 %>% 
  select(edad,
         sexo,
         region, 
         expr, 
  ...
  rename(ingreso = yoprcor) %>%
  mutate(año = 2015,
         ############### Inicio de Transformación de Valores Missing en NA ##############
         o10 = replace(o10,o10== 999, NA), 
         e6a = replace(e6a, e6a == 99, NA),
         y2h = replace(y2h, y2h == 999, NA), 
         ...
         ocupacion_formal = case_when(
           ...
           o15 == 9  | s12 == 8 | o29 == 7  ~ "0" , # Ocupación Informal
           TRUE ~ NA_character_
         ), 
